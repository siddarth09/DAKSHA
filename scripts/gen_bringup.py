...
def controllers_yaml(key: str) -> str:
    ...
    # ── F/T BROADCASTERS, one per fingertip sensor. The plugin exposes each wrench as six
    # state interfaces; nothing reaches a topic until a broadcaster reads them, so an F/T sensor
    # that exists in both descriptions still publishes NOTHING without this -- the same silent
    # by-name binding failure as the cameras.
...
def control_yaml(key: str) -> str:
    ...
    if mjcf.exists():
        ...
        # The can is a primitive built by gen_scene, so its body is `obj_root`; the tray is
        # still an attached asset whose mesh sits on `plate_object`. Asserting the old
        # `obj_object` name here just meant this generator raised and left the previous, now
        # wrong, pick_xyz in place -- stale coordinates are worse than a crash, so keep it fatal
        # but look for the right body.
        ...
        # TELEOP WORKSPACE BOX, in table coordinates. A gamepad integrates velocity into a
        # target pose, so without a clamp the target keeps travelling after the arm has stopped
        # -- push into the table for two seconds and the target is 20 cm underneath it, the IK
        # residual is huge, and the arm no longer answers the stick until you drive all the way
        # back out. Bounds come from the table, not from taste. The z floor sits one object-height
        # above the surface so the gripper can reach a resting object but not through it.
    ...
    # Camera and F/T names for any node that wants to subscribe to all of them (the rerun
    # viewer, the dataset recorder). Generated so nobody maintains a second list.
