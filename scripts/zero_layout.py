...
# ⚠️ EVERY CAMERA COSTS FRAME RATE, and the dataset can never be faster than its slowest
# observation. Measured with 5 cameras: 5.00 Hz at 640x480, 5.60 at 320x240, 6.50 at 224x224 --
# so resolution is almost free and each extra camera is not. Down to front + two wrists.
...
# Camera publish rate, and it has to be set explicitly. mujoco_ros2_control defaults to 5 Hz
# (measured 4.6), which is fine for eyeballing the sim and far too slow to imitate from -- an
# operator's corrections simply are not in the data at 5 Hz. Read as a HARDWARE parameter from
# the URDF's <ros2_control><hardware> block (mujoco_system_interface.cpp reads
# `camera_publish_rate` there), so this is ours to set without touching that package. It also
# caps the LeRobot dataset fps: the dataset can never be faster than its slowest observation.
CAM_RATE_HZ = 30.0
...
# AN UPRIGHT CAN, and specifically a UNIFORM CYLINDER. Two separate requirements, learned the
# hard way:
#
#   Height. The original lemon wedge lies flat -- 23 mm tall, grasp centre ~11 mm off the table --
#   so a top-down gripper has to reach almost to the surface, which put the reBot's left arm on
#   its workspace boundary. An upright object carries its grasp centre ~75 mm up and the reBot
#   reaches it comfortably.
#
#   Shape. Juice006 satisfied the height requirement and still failed in teleop, because it is a
#   BOTTLE: measured, its diameter is a uniform 49 mm up to 58% of its height and then breaks into
#   a shoulder and a 35 mm cap. Any grasp above mid-body closes on the neck, so the operator gets
#   a hold on the cap instead of the body. A summary "uniformity" statistic over the whole grasp
#   band hid this -- the diameter PROFILE showed it immediately.
#
# Paprika012 measures 64-67 mm across from 5% to 75% of its height: a real cylinder, so any grasp
# in that band is on the body. Verified on both embodiments (grasp at 75% height, then lifted and
# translated 80 mm to stress the hold):
#   reBot  lift 100.2 mm, in-jaw slip 6.2 mm     Panda  lift 87.7 mm, slip 12.6 mm
# The near-cylinders that are cleaner in profile are not usable: Cinnamon007/005 tip Panda over
# at every grasp height, GlassCup025 tips the reBot. This is the only candidate both arms hold.
...
#   reBot : gripper 100 mm open (measured: two 50 mm slide joints, coupled by an equality)
#   Dex3  : gripper 124 mm open (thumb-tip to index-tip)
# The binding aperture is PANDA'S 80 mm -- the reBot opens wider, not narrower, contrary to an
# earlier note here. Panda's measured throat at full open is 74 mm, so a 66 mm object leaves only
# ~4 mm per side: it grasps reliably in test but this is the tight constraint to respect when
# swapping the object, not the reBot.
# ⚠️ GRIPPER FORCE LIMITS, because the source models effectively have none: the reBot's gripper
...
# holding -- measured 11 mm of penetration into a 66 mm can, which crushes it, squirts it out of
# the jaws, and makes contact forces meaningless. Real values make the grasp compliant: it
# squeezes until the limit and then holds by friction, which is also what the recorded demo should
# show. Applied by gen_scene to the gripper actuators only; the arm joints keep their own limits.
OBJECT_WIDTH = 0.066

# Mass of the pick object, forced rather than inherited. The robocasa assets are meshes with a
# default density, so a spice tin came out at 0.763 kg -- roughly twice a full 330 ml can, and
# heavy enough on a small arm that the grasp keeps failing under load. A real coke can is ~0.39 kg
# full, ~0.015 kg empty; 0.39 keeps it a plausible payload for both embodiments while still being
# something the gripper has to actually hold. Set in the MJCF at generation time: writing mass or
# friction onto an already-compiled model measures byte-identical, the same trap as body_gravcomp.
# EMPTY can (~15 g of aluminium), not a full one (~350 g). The asset it replaced weighed
# 1.19-1.51 kg -- the mass is split across a nested body, so reading one body's own mass
# understates it -- which is simply too much payload to keep hold of on a small arm.
# Being this light costs nothing in stability, which is not obvious and was worth measuring: the
# 7.6 g lemon MESH rocked through 24-58 deg because MuJoCo's convex-convex narrowphase gave it a
# single contact point, whereas this cylinder is a primitive and collides analytically -- drift
# over 20 s is 0.00 mm / 0.00 deg at every mass from 15 g to 350 g. Set PICK_MASS = 0.35 for a
# full can if a heavier payload is ever wanted.
PICK_MASS = 0.015

# ── THE PICK OBJECT IS A BUILT PRIMITIVE, not a robocasa asset. ────────────────────────────
# There is no drinks can in the 69-object lightwheel set, and every asset near the right size is
# the wrong shape or the wrong proportions, measured:
...
# A real 330 ml can is 66 x 115 mm, aspect 1.74 -- squatter than anything available, which is
# exactly why it stands up to being grasped. Two further wins over a mesh: a cylinder primitive
# collides analytically instead of through a convex hull (no multiccd contact-count games), and
# its friction is ours to set, where every robocasa asset ships torsional friction 0.005 and a
# post-compile write to change that is silently ignored.
# Mesh used purely for the can's APPEARANCE -- collision and mass come from the cylinder in
# PICK_PRIMITIVE. Scaled onto that cylinder at generation time, so swapping this changes only how
# the object looks and never how it behaves. Set to None for the bare primitive.
PICK_SKIN = None
...
PICK_PRIMITIVE = {
    ...
    # Contact time constant. MuJoCo's default 0.02 is deliberately soft for solver stability, and
    # on a grasped object that softness is visible: the pads sank 7.8 mm into a 66 mm can, which
    # looks exactly like the gripper passing through it. 0.005 with a realistic gripper force
    # brings that to 2.2 mm -- 3% of the diameter, invisible -- and the lift is unaffected (76 mm
    # either way), so the softness was buying nothing here.
    "solref": (0.005, 1.0),
}
...
ROBOTS = {
    "rebot": {
        ...
        # ⚠️ MEASURED, not chosen -- scripts/measure_tcp.py, asserted by check_parity.py. The
        # offset from `gripper_end` to where the two pads MEET when closed. Two wrong values
        # preceded this one, both of which produced grasps that worked often enough to look like
        # a tuning problem: (0, 0, 0.10), copied from Panda's convention, put the commanded point
        # 113 mm out in mid-air beside the gripper; then the midpoint of the finger collision
        # AABBs put it 39 mm off, because these finger meshes are asymmetric. The symptom of the
        ...
        "grip_range": (0.0, 0.05),
        "grip_force": 15.0,   # N, small servo gripper -- see below   # reBot fingers are SLIDE joints, range [0, 0.05]
    ...
    "panda": {
        ...
        "eef_body": "hand",
        # MEASURED (scripts/measure_tcp.py). The guessed 0.10 was 17.9 mm long -- not enough to
        # stop Panda's 80 mm fingers closing on things, but a constant bias in every recorded
        ...
        "grip_range": (0.0, 0.04),
        "grip_force": 40.0,   # N -- see below   # Panda finger_joint1 range [0, 0.04]
...
def ft_sensors(key: str) -> list[tuple[str, str]]:
    ...
    each finger's own joint, which is what tells you whether both pads are loaded or the object
    is sitting against one of them. (That asymmetry is exactly the failure the TCP bug produced,
    and a per-finger sensor would have shown it immediately: +9.5 mm of clearance on one pad and
    +99 mm on the other.)

    ⚠️ NAMING IS A CONTRACT WITH mujoco_ros2_control. It resolves a `<sensor mujoco_type="fts">`
...
# (PICK_PRIMITIVE) rather than taken from an asset; the tray is a robocasa mesh. Sizes measured
# from mesh vertices, never geom_rbound -- that reports a bounding-SPHERE radius and made the
# flat tray read as 460 mm cubed.
