...
1. **Mark what has actually been read.** Every note carries a `Status:` line, `abstract read` vs `full paper read`. Notes written from an abstract flag anything unverified. It is very easy to end up citing a number nobody checked.
...
### Our mechanism is the field standard, so read these before claiming novelty
