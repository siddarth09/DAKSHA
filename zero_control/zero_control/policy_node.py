...
class PolicyNode(Node):
    def __init__(self) -> None:
        ...
        # EMPTY MEANS "take it from the dataset". The language instruction is part of the
        # conditioning, so a mismatch between training and inference is silent degradation that
        # looks exactly like a policy failure -- and the default here was v1's string ("pick up
        # the can and place it in the tray") while cross_v1 was recorded with a longer one that
        # names the handover. Reading it from the metadata removes the whole class of bug.
        ...
        # Dump every tick to an npz for offline analysis. The closed loop cannot be reproduced
        # offline -- the images depend on where the arm actually went -- so when a rollout
        # misbehaves this trace is the only way to see the measured state and the commanded action
        # side by side over time.
        self.declare_parameter("trace_path", "")
        # Mirage uses a "high-gain or blocking controller on the target robot" and enforces that
        # the norm of the error from the desired pose is < 0.015 m at every timestep. Our servo is
        # a one-step DLS chase: measured, a 70-86 mm move needs ~20 ticks (200 ms) to settle but
        # gets 100 ms at 10 Hz, so it never catches up -- that is the 22-41 mm residual seen in
        # rollout. Blocking decouples policy rate from settling time. It makes the rollout slower
        # than real time, which is fine in sim, and it is what makes `action` == achieved pose (so
        # no forward dynamics model is needed).
        self.declare_parameter("block_until_settled", True)
        self.declare_parameter("settle_m", 0.015)      # Mirage's position tolerance
        # Mirage bounds the norm of the pose error INCLUDING the quaternion, so orientation
        # has to gate the wait too -- /zero/ik_status carries rot_err at indices 1 and 4.
        self.declare_parameter("settle_rad", 0.03)     # ~1.7 deg
        self.declare_parameter("settle_timeout_s", 1.0)
        # NOTE: blocking and a short n_action_steps work against each other -- at 5 steps the
        # target moves every 0.5 s and the arm cannot settle inside the tolerance, giving 64
        # stalls in 106 s. Measured, the short horizon did not help anyway (closest approach
        # 61.5 mm vs 57.0 mm at 25 steps), so prefer 25 and leave blocking on.
        ...
        # ⚠️ REFUSE TO RUN AGAINST A COMPETING PUBLISHER. teleop_node publishes the SAME topic at
        # 50 Hz and, with no stick input, holds the home pose -- so a teleop left running from a
        # recording session silently overwrites every policy command 5x per tick. Measured: the
        # policy asked for 162 mm of motion and the arm moved 2.3 mm in 106 s, while `resid` read
        # 1 mm because the arm was tracking teleop's home target perfectly. That is invisible from
        # this node's own logs, which is exactly why it needs a guard rather than a comment.
    ...
    def _tick(self) -> None:
        ...
        if self.blocking and self.ticks and (self.residual > self.settle_m
            ...
            # Hold the current target until the arm arrives, rather than issuing a new one the
            # servo has no chance of reaching. Timeout so an unreachable pose cannot deadlock.
    ...
    def _on_status(self, msg: Float64MultiArray) -> None:
        ...
        # tick-based gate fires on every 100 Hz status message and floods the log -- which is what
        # buried the useful lines in the 5-step rollout.
